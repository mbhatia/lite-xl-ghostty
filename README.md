...
- Lite XL 2.1.x.
- For binary installs: macOS arm64 (`aarch64-darwin`), macOS Intel
  (`x86_64-darwin`), Linux x64 (`x86_64-linux`), or Linux arm64
  (`aarch64-linux`).
...
Binary releases are available for macOS arm64, macOS Intel, Linux x64, and
Linux arm64:
...
`ghostty` depends on the `ghostty_lxl` library addon. LPM downloads the release
asset matching the current architecture and installs it as
`libraries/ghostty_lxl/init.lib` on macOS or `libraries/ghostty_lxl/init.so` on
Linux, which is what the Lua plugin requires.

Windows binary packages are not published yet.
...
The `release-binaries` GitHub Actions workflow builds on native macOS and Linux
runners for all published LPM architectures. It checks out the pinned Ghostty
source used by this repo's CMake integration, builds and tests the native
module, stages `dist/ghostty_lxl.<arch>.<ext>`, writes
`dist/ghostty_lxl.<arch>.<ext>.sha256`, and uploads both as workflow artifacts.
Tag pushes matching `v*` also upload those files to the GitHub release.

The `v0.2.0` manifest points at the published release assets and includes their
SHA-256 checksums. For future releases, update `manifest.json` with the
checksums generated by the tag workflow before announcing LPM installs. Do not
reuse a published tag with a different binary after users can install it.
